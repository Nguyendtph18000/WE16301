<?php
    function list_tour_avtive(){
        $sql = "SELECT * FROM tours WHERE status=0 order by id desc";
        $tours = executeQuery($sql,true);

        include_once "./views/admin/layouts/tour/tour-active.php";
    }
    function list_tour_pending(){
        $sql = "SELECT * FROM tours WHERE status=1 order by id desc";
        $tours = executeQuery($sql,true);
        include_once "./views/admin/layouts/tour/tour-pending.php";
    }
    function list_tour_expired(){
        $sql = "SELECT * FROM tours WHERE status=2 order by id desc";
        $tours = executeQuery($sql,true);
    ...
    function add_tour(){
        ...
        if(isset($_POST['publish'])){
            ...
            $sql = "INSERT INTO tours (destination,people,days,nights,price,discount,image,title,description,detail_photo,start_day,end_day) VALUES('$destination','$people','$days','$nights','$price','$discount','$image','$title','$description','$image_detail','$start_day','$end_day')  ";
            executeQuery($sql);

            // header("location: tour-active");
        }
    }
    ...
    function edit_tour(){
        if(isset($_GET['id'])){
            ...
            if(isset($_POST['publish'])){
                ...
                $file_img_detail = $_FILES['image-tour-detail']?$_FILES['image-tour-detail']:"";
    
                if($file_img_detail['size'] > 0){
                    $filenamedetail = $file_img_detail['name'];
                    move_uploaded_file($file_img_detail['tmp_name'], './public/homepage/assets/images/' . $filenamedetail);
                    $image_detail = $filenamedetail;
                }
                else{
